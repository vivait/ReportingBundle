...
class BarChart extends Chart
{
    ...
    {
        return 'VivaitReportingBundle:Chart:barchart.html.twig';
    }
    ...
    public function getOptions(array $columnmapping)
    {
        ...
        if (count($this->getDimensions($columnmapping))) {
            $x_axis = current($this->getDimensions($columnmapping));
        }
    ...
    public function getGraphData(array $data, array $columnmapping)
    {
        ...
        if ($dimension_count == 2) {
            ...
            foreach (array_values(array_unique(array_column($data, $x2_axis))) as $dimension) {
                ...
                $color_hex = substr(sha1($dimension), 0, 6);
                $color_rgb = hexdec($color_hex[0] . $color_hex[1]) . ',' . hexdec($color_hex[2] . $color_hex[3]) . ',' . hexdec($color_hex[4] . $color_hex[5]);
